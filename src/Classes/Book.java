...
public class Book extends Item{
    ...
    private final List<requestsForHold> onHoldRequests; // all the hold requests for this book
    ...
    public int processHoldRequestForBook(Borrower borrower)
    {
        ...
        // case 2
        for (requestsForHold onHoldRequest : this.onHoldRequests) {
            if (onHoldRequest.getBorrower() == borrower) {
    ...
    public int checkOutThisBook(Borrower borrower) // issue
    {
        ...
        else
        {
            if (!onHoldRequests.isEmpty()) // checking if the current borrower is first in queue of reservation
            {
                ...
                for (requestsForHold onHoldRequest : onHoldRequests) {
                    if (onHoldRequest.getBorrower().getId() == borrower.getId()) {
                        requested = true;
                    }
                }
                if (requested == true)
